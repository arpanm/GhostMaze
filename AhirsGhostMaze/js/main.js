...
function startGame() {
    ...
    if (!economy.hasEnoughBalance(10)) {
        ...
        document.getElementById('low-balance-modal').style.display = 'flex';
        return;
    }
...
function endGame(win, reason = "UNKNOWN") {
    ...
    // Coins Reward - Removed per requirement
    /*
    const coinsEarned = Math.floor(finalScore / 100);
    if (coinsEarned > 0) {
        economy.addCoins(coinsEarned, 'Maze Reward');
        updateMenuBalance();
        document.getElementById('final-score').insertAdjacentHTML('beforeend', `<span style="color:gold; font-size:1rem; margin-left:10px;">(+${coinsEarned} 🪙)</span>`);
    }
    */
